...
function setup_channel_benchmark(num_samples::Int, buffer_size::Int)
    ...
    # Pre-allocate data once
    data = rand(ComplexF32, num_samples, 1)
    return (ch, data)
end

# Benchmark: push data through the channel and drain output
function run_channel_benchmark!(ch::SignalChannel{ComplexF32}, data::Matrix{ComplexF32}, num_items::Int)
    # Producer task
